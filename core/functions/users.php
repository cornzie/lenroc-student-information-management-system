...
function activate($email, $email_code){
	...
	if(mysql_result(mysql_query("SELECT COUNT(`user_id`) FROM `users` WHERE `email`='$email' AND `email_code`='$email_code' AND `active`='d'"), 0)==1){
		mysql_query("UPDATE `users` SET `active`='a' WHERE `email`='$email'");
		return true;
	} else{
		return false;
	}
...
function register_user($register_data){
	...
	email($register_user['email'], 'Activate your account', "Hello ". $register_data['firstname'].",\n\n You need to activate your account. please click on the link below:\n\n http://localhost/lenroc/activate.php?email=".$register_data['email']."&email_code=". $register_data['email_code']."\n\n-Lenroc Inc.");
}

function user_count(){
	return mysql_result(mysql_query("SELECT COUNT(`user_id`) FROM `users` WHERE `active`='a'"), 0);
}
...
$username=sanitize($username);
$query=mysql_query("SELECT COUNT(`user_id`) FROM`users` WHERE `username`='$username' AND `active`='a'");
return( mysql_result($query, 0)==1) ? true : false;
}
